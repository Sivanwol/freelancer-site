...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/${locale}${sitePaths.termsOfUse}`,
      languages: {
        he: `${baseUrl}/he${sitePaths.termsOfUse}`,
        en: `${baseUrl}/en${sitePaths.termsOfUse}`,
      },
    },
  };
}
            ...
            <div className="site-container relative z-10 pb-12 pt-10">
              <div className="max-w-4xl">
                ...
                <h1 className="hero-display text-5xl font-black leading-[0.9] text-[#0d1626] md:text-7xl">
                  {content.termsPage.title}
                </h1>
                <p className="mt-5 text-sm font-extrabold text-[#1d72d2]">{content.termsPage.updated}</p>
                <p className="mt-7 max-w-3xl text-lg font-semibold leading-8 text-[#526174] md:text-xl md:leading-9">
                  {content.termsPage.intro}
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="site-section bg-white">
            <div className="site-container">
              <div className="grid gap-5">
                {content.termsPage.sections.map((section, index) => (
                  <article
                    key={section.title}
                    className="rounded-[28px] border border-[#dbe7f5] bg-[#f8fbff] p-6 shadow-sm md:p-8"
                  >
